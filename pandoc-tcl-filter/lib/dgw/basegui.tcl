...
snit::type ::dgw::basegui {
    ...
    #' 
    #'   __-title__ _boolean_ 
    #' 
    #'  > Set the application title, default DGBaseGui. 

    option -title "DGBaseGui"
    variable var
    variable gui
    variable x
    variable top
    ...
    method init {} {
        ...
        set var(revision) [package provide basegui]
        set var(year) 2022
        set var(appinfo) "" 
    ...
    #'
    #' <a name="setAppname">*cmdName* **setAppname*  *appname revision author year ?info?*</a>
    #' 
    #' > Sets the application time of the toplevel window and for the About boxes.
    #'   In the About box as well the optional info is shown.
    #'
    #' > ```
    #' package require dgw::basegui
    #' dgw::basegui app
    ...
    #' set rotext [app rotext $roframe.rotext -background salmon]
    #' app autoscroll $rotext
    #' $rotext ins end "Hello setAppname Demo"
    #' app setAppname RoTextDemo 0.0.1 NN 2022
    #' pack $roframe -side top -fill both -expand true
    #' > ```
    #' 

    method setAppname {appname revision author year {info ""}} {
        set var(appname) $appname
        set var(revision) $revision
        set var(author) $author
        set var(year) $year
        set var(appinfo) $info        
        wm title $top "$var(appname), $var(revision), by @ $var(author), $var(year)"
    }
    #'
